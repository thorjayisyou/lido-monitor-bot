...
def job():#把所有业务逻辑包进job函数，schedule每小时调用一次这个函数。
    ...
    #计算三源最大偏差和价差预警
    price_diff = abs(latest['eth_price_chainlink'] - latest['eth_price_pyth'])
    three_tvl = [latest['total_tvl_chainlink'], latest['total_tvl_pyth'], latest['Defillama_tvl']]
    three_deviation = max(three_tvl) - min(three_tvl)
    three_deviation_pct = three_deviation / min(three_tvl) * 100
    price_diff_alert = "⚠️ 价差异常！" if price_diff > 5 else "✅ 价差正常"
    three_alert = "⚠️ 偏差较大" if three_deviation_pct > 1 else "✅ 正常"

    #有历史数据才显示各项变化，没有就留空
    if has_hourly:
        cl_chg  = f"   {arrow(eth_change_chainlink)} *{eth_change_chainlink:+,.2f}* ({eth_change_chainlink_pct:+.2f}%)"
        py_chg  = f"   {arrow(eth_change_pyth)} *{eth_change_pyth:+,.2f}* ({eth_change_pyth_pct:+.2f}%)"
        sup_chg = f"   {arrow(supply_change)} *{supply_change:+,.2f} ETH*"
        st_chg  = f"   {arrow(steth_tvl_change)} *${steth_tvl_change:+,.2f}*"
        ot_chg  = f"   {arrow(other_tvl_change)} *${other_tvl_change:+,.2f}*"
        summary = (
            f"*[ 小时变化汇总 ]*\n"
        ...
        cl_chg = py_chg = sup_chg = st_chg = ot_chg = ""
        summary = f"*[ 预警 ]*\n{alert}"
    ...
    if today_real_8am is None or yesterday_real_8am is None:#如果几天或者昨天的时间锚定点里面任中返回一个空集，就是说有数据找不到，那么就没办法进行日环比计算。
        ...
        if tvl_change_chainlink_percentage_real_8am < -10:
            daily_tvl_alert = "🔴大户都在跑路了，快跑！"
        elif tvl_change_chainlink_percentage_real_8am < -5:
            daily_tvl_alert = "🟡可能存在风险，需要注意了。"
        else:
            daily_tvl_alert = "✅数据正常，该吃吃该喝喝。"
        print(f"链上总锁仓量日环比变化：{daily_tvl_alert}")
        ...
        if today_real_8am.get("apr_onchain") is not None and yesterday_real_8am.get("apr_onchain") is not None:
            ...
            print(f"链上APR变化：{apr_onchain_yesterday*100:.4f}% -> {apr_onchain_today*100:.4f}%，日环比：{apr_onchain_change_pp:+.4f}pp")

            if apr_onchain_change_pp < -1:
                daily_apr_alert = "🔴 红色预警：收益率暴跌！"
            elif apr_onchain_change_pp < -0.5:
                daily_apr_alert = "🟡 黄色预警：需要关注。"
            elif apr_onchain_change_pp > 1:
                daily_apr_alert = "🔴 红色预警：异常暴涨疑似刷数据！"
            elif apr_onchain_change_pp > 0.5:
                daily_apr_alert = "🟡 黄色预警：需要关注。"
            else:
                daily_apr_alert = "✅ 正常，无需预警。"
            print(f"链上APR日环比预警：{daily_apr_alert}")

            daily_apr_line = (
                f"昨天：{apr_onchain_yesterday*100:.4f}%  →  今天：*{apr_onchain_today*100:.4f}%*\n"
                f"日环比：*{apr_onchain_change_pp:+.4f}pp*\n"
                f"预警：*{daily_apr_alert}*"
            )
        else:
            print("APR历史数据不足，跳过日环比预警")
            daily_apr_line = "APR历史数据不足，暂无日环比数据"

        #stETH供应量日环比
        steth_supply_change = today_real_8am["steth_supply_eth"] - yesterday_real_8am["steth_supply_eth"]
