...
def send_tg(message):#定义发送Telegram消息的函数，后面需要发消息的地方直接调用这个函数就好。
    ...
    if not token or not chat_id:#如果token或chat_id没有填，就跳过发送，不让程序崩溃。
        ...
        return#让程序不崩溃，继续运行下面的步骤。
    try:#try/except语法很有意思，相当于try里面的内容如果正常运行了，就不继续运行except里面的内容，如果try里面的内容运行失败了，就继续运行except里面的内容。表示一种备用选择，而不是try里面都调用失败就把程序搞崩溃，用except里面的步骤来当失败托底，就是说失败了我们就跳过这个事情别纠结这个程序运行，继续往下面走就好。
        url = f"https://api.telegram.org/bot{token}/sendMessage"#这是tg提供的直接和机器人对话的接口，我们把token和chat_id写成一个url，方便后面引用。
        resp = requests.post(url, data={"chat_id": chat_id, "text": message, "parse_mode": "Markdown"}, timeout=10)#resp是response的缩写，表示响应的意思，我们用requests.post请求url，data是请求的数据，{"chat_id": chat_id, "text": message, "parse_mode": "Markdown"}是请求的数据，chat_id是聊天id，message是发送的消息，parse_mode是解析模式，timeout是请求超时时间。
        result = resp.json()#result是响应的结果，我们用resp.json()把响应的结果转换成json格式，方便后面引用。
        if result.get("ok"):#如果result.get("ok")为True，就表示发送成功，就执行下面步骤。
            print("✅ TG消息发送成功")
        else:#如果result.get("ok")为False，就表示发送失败，就执行下面步骤。
            print(f"⚠️ TG消息发送失败，TG返回错误：{result}")
    except Exception as e:#这个是当try里面的步骤失败了，提醒失败原因的语法，Exception代表所有错误类型的集合，把遇到的错误类型定义叫e，后面打印失败原因的时候直接引用。
...
def job():#把所有业务逻辑包进job函数，schedule每小时调用一次这个函数。
    ...
    # 注意：DefiLlama有时返回WMATIC，有时返回MATIC，用get兼容两种情况，找不到则为0
    WMATIC_TVL = tokens_usd.get("WMATIC") or tokens_usd.get("MATIC", 0)#兼容WMATIC和MATIC两种键名。
    DOT_TVL = tokens_usd.get("DOT", 0)#找不到DOT则为0，防止KeyError。
    KSM_TVL = tokens_usd.get("KSM", 0)#找不到KSM则为0，防止KeyError。
    SOL_TVL = tokens_usd.get("SOL", 0)#找不到SOL则为0，防止KeyError。
    ...
    if len(history1) < 2:#历史数据不足，跳过小时预警分析
        print("历史数据不足，跳过小时预警分析")
        has_hourly = False
        alert = "历史数据不足，暂无小时变化数据"
        eth_change_chainlink = eth_change_chainlink_pct = 0
        eth_change_pyth = eth_change_pyth_pct = 0
        supply_change = supply_change_pct = 0
        tvl_hourly_change = tvl_hourly_change_percentage = 0
        steth_tvl_change = other_tvl_change = 0
    else:
        has_hourly = True
        previous = history1[-2]#取倒数第二条数据。
        latest = history1[-1]#取倒数第一条数据，也就是刚刚运行写入这条。

        eth_change_chainlink = latest["eth_price_chainlink"] - previous["eth_price_chainlink"]#ETH价格小时变化额（chainlink）
        eth_change_chainlink_pct = eth_change_chainlink / previous["eth_price_chainlink"] * 100#ETH价格小时变化幅度（chainlink）
        eth_change_pyth = latest["eth_price_pyth"] - previous["eth_price_pyth"]#ETH价格小时变化额（pyth）
        eth_change_pyth_pct = eth_change_pyth / previous["eth_price_pyth"] * 100#ETH价格小时变化幅度（pyth）
        supply_change = latest["steth_supply_eth"] - previous["steth_supply_eth"]#供应量小时变化额
        supply_change_pct = supply_change / previous["steth_supply_eth"] * 100#供应量小时变化幅度
        tvl_hourly_change = latest["total_tvl_chainlink"] - previous["total_tvl_chainlink"]#总TVL小时变化额
        tvl_hourly_change_percentage = tvl_hourly_change / previous["total_tvl_chainlink"] * 100#总TVL小时变化幅度
        steth_tvl_change = latest["steth_tvl_chainlink"] - previous["steth_tvl_chainlink"]#stETH TVL小时变化额
        other_tvl_change = latest["other_tvl"] - previous["other_tvl"]#其他代币TVL小时变化额

        print(f"TVL变化额：${tvl_hourly_change:,.2f}")
        print(f"TVL变化幅度：{tvl_hourly_change_percentage:.2f}%")

        if tvl_hourly_change_percentage < -2:
            alert = "🔴大户都在跑路了，快跑！"
        elif tvl_hourly_change_percentage < -1:
            alert = "🟡可能存在风险，需要注意了。"
        else:
            alert = "✅数据正常，该吃吃该喝喝。"
        print(f"链上总锁仓量在过去1小时内变化：{alert}")

    #计算三源最大偏差和价差预警
    price_diff = abs(latest['eth_price_chainlink'] - latest['eth_price_pyth'])#chainlink和pyth求出价格差，然后取绝对值。
    three_tvl = [latest['total_tvl_chainlink'], latest['total_tvl_pyth'], latest['Defillama_tvl']]#把chainlink、pyth、defillama三个来源的tvl值放到一个列表里面，然后命名为three_tvl。
    three_deviation = max(three_tvl) - min(three_tvl)#three_tvl列表里面找出最大值和最小值，然后相减，得到最大偏差值，然后命名为three_deviation。
    three_deviation_pct = three_deviation / min(three_tvl) * 100#three_deviation除以three_tvl列表里面的最小值，然后乘以100，得到最大偏差百分比，然后命名为three_deviation_pct。
    price_diff_alert = "⚠️ 价差异常！" if price_diff > 5 else "✅ 价差正常"#如果price_diff大于5，则提示价差异常，否则提示价差正常。
    three_alert = "⚠️ 偏差较大" if three_deviation_pct > 1 else "✅ 正常"#如果three_deviation_pct大于1，则提示偏差较大，否则提示正常。

    #有历史数据才显示各项变化，没有就留空
    if has_hourly:
        cl_chg  = f"   {arrow(eth_change_chainlink)} *{eth_change_chainlink:+,.2f}* ({eth_change_chainlink_pct:+.2f}%)"#chainlink价格变化额，变化幅度。
        py_chg  = f"   {arrow(eth_change_pyth)} *{eth_change_pyth:+,.2f}* ({eth_change_pyth_pct:+.2f}%)"#pyth价格变化额，变化幅度。
        sup_chg = f"   {arrow(supply_change)} *{supply_change:+,.2f} ETH*"#供应量变化额，变化幅度。
        st_chg  = f"   {arrow(steth_tvl_change)} *${steth_tvl_change:+,.2f}*"#stETH TVL变化额，变化幅度。
        ot_chg  = f"   {arrow(other_tvl_change)} *${other_tvl_change:+,.2f}*"#其他代币TVL变化额，变化幅度。
        summary = (
            f"*[ 小时变化汇总 ]*\n"#小时变化汇总标题。
            f"ETH价格  {arrow(eth_change_chainlink)} *${eth_change_chainlink:+,.2f}*  ({eth_change_chainlink_pct:+.2f}%)\n"#chainlink价格变化额，变化幅度。
            f"供应量   {arrow(supply_change)} *{supply_change:+,.2f} ETH*  ({supply_change_pct:+.4f}%)\n"#供应量变化额，变化幅度。
            f"总TVL    {arrow(tvl_hourly_change)} *${tvl_hourly_change:+,.2f}*  ({tvl_hourly_change_percentage:+.2f}%)\n"#总TVL变化额，变化幅度。
            f"\n"#空行。
            f"*[ 预警 ]*\n"#预警标题。
            f"{alert}"#预警内容。
        )
    else:
        cl_chg = py_chg = sup_chg = st_chg = ot_chg = ""#如果历史数据不足，则把所有变量都赋值为空字符串。
        summary = f"*[ 预警 ]*\n{alert}"#如果历史数据不足，则把预警内容赋值为空字符串。 

    tg_hourly_msg = (#把要发到tg的小时变化相关数据按以下格式布置
        f"📊 *Lido 小时数据速报*\n"
        f"{'─'*28}\n"
        f"🕐 {latest['timestamp']}\n"
        f"{'─'*28}\n"
        f"*[ ETH 价格 ]*\n"
        f"Chainlink : *${latest['eth_price_chainlink']:,.2f}*{cl_chg}\n"
        f"Pyth      : *${latest['eth_price_pyth']:,.2f}*{py_chg}\n"
        f"两者价差  : ${price_diff:.4f}   {price_diff_alert}\n"
        f"\n"
        f"*[ stETH 供应量 ]*\n"
        f"链上总量  : *{latest['steth_supply_eth']:,.2f} ETH*{sup_chg}\n"
        f"\n"
        f"*[ TVL 构成（Chainlink 口径） ]*\n"
        f"stETH TVL : *${latest['steth_tvl_chainlink']:,.2f}*{st_chg}\n"
        f"其他代币  : *${latest['other_tvl']:,.2f}*{ot_chg}\n"
        f"合计总TVL : *${latest['total_tvl_chainlink']:,.2f}*\n"
        f"\n"
        f"*[ TVL 多源对比 ]*\n"
        f"Chainlink : *${latest['total_tvl_chainlink']:,.2f}*\n"
        f"Pyth      : *${latest['total_tvl_pyth']:,.2f}*\n"
        f"DefiLlama : *${latest['Defillama_tvl']:,.2f}*\n"
        f"三源最大偏差 : ${three_deviation:,.2f}（{three_deviation_pct:.2f}%）  {three_alert}\n"
        f"{'─'*28}\n"
        f"{summary}\n"
        f"{'─'*28}"
    )
    print(tg_hourly_msg)#打印tg_hourly_msg，方便我们查看。
    send_tg(tg_hourly_msg)#把这些数据产生放进send_tg函数里面，然后发到tg。
    ...
    if today_real_8am is None or yesterday_real_8am is None:#如果几天或者昨天的时间锚定点里面任中返回一个空集，就是说有数据找不到，那么就没办法进行日环比计算。
        ...
        if tvl_change_chainlink_percentage_real_8am < -10:
            daily_tvl_alert = "🔴大户都在跑路了，快跑！"#如果tvl_change_chainlink_percentage_real_8am小于-10，则提示大户都在跑路了，快跑！
        elif tvl_change_chainlink_percentage_real_8am < -5:
            daily_tvl_alert = "🟡可能存在风险，需要注意了。"#如果tvl_change_chainlink_percentage_real_8am小于-5，则提示可能存在风险，需要注意了。
        else:
            daily_tvl_alert = "✅数据正常，该吃吃该喝喝。"#如果tvl_change_chainlink_percentage_real_8am大于等于-5，则提示数据正常，该吃吃该喝喝。
        print(f"链上总锁仓量日环比变化：{daily_tvl_alert}")#打印链上总锁仓量日环比变化，方便我们查看。

        #以下是日环比apr预警分析代码
        if today_real_8am.get("apr_onchain") is not None and yesterday_real_8am.get("apr_onchain") is not None:
            apr_onchain_today = today_real_8am.get("apr_onchain")#用apr_onchain_today定义今天早上8点的apr。
            apr_onchain_yesterday = yesterday_real_8am.get("apr_onchain")#用apr_onchain_yesterday定义昨天早上8点的apr。
            apr_onchain_change_pp = (apr_onchain_today - apr_onchain_yesterday) * 100  # 绝对百分点差
            print(f"链上APR变化：{apr_onchain_yesterday*100:.4f}% -> {apr_onchain_today*100:.4f}%，日环比：{apr_onchain_change_pp:+.4f}pp")#打印链上APR变化，方便我们查看。

            if apr_onchain_change_pp < -1:
                daily_apr_alert = "🔴 红色预警：收益率暴跌！"#如果apr_onchain_change_pp小于-1，则提示收益率暴跌！
            elif apr_onchain_change_pp < -0.5:
                daily_apr_alert = "🟡 黄色预警：需要关注。"#如果apr_onchain_change_pp小于-0.5，则提示需要关注。
            elif apr_onchain_change_pp > 1:
                daily_apr_alert = "🔴 红色预警：异常暴涨疑似刷数据！"#如果apr_onchain_change_pp大于1，则提示异常暴涨疑似刷数据！
            elif apr_onchain_change_pp > 0.5:
                daily_apr_alert = "🟡 黄色预警：需要关注。"#如果apr_onchain_change_pp大于0.5，则提示需要关注。
            else:
                daily_apr_alert = "✅ 正常，无需预警。"#如果apr_onchain_change_pp大于等于-0.5，则提示正常，无需预警。
            print(f"链上APR日环比预警：{daily_apr_alert}")#打印链上APR日环比预警，方便我们查看。

            daily_apr_line = (
                f"昨天：{apr_onchain_yesterday*100:.4f}%  →  今天：*{apr_onchain_today*100:.4f}%*\n"#昨天和今天的apr，日环比。
                f"日环比：*{apr_onchain_change_pp:+.4f}pp*\n"
                f"预警：*{daily_apr_alert}*"#预警内容。
            )
        else:
            print("APR历史数据不足，跳过日环比预警")#打印APR历史数据不足，跳过日环比预警，方便我们查看。
            daily_apr_line = "APR历史数据不足，暂无日环比数据"#把APR历史数据不足，暂无日环比数据赋值给daily_apr_line。

        #stETH供应量日环比
        steth_supply_change = today_real_8am["steth_supply_eth"] - yesterday_real_8am["steth_supply_eth"]#用steth_supply_change定义今天和昨天早上8点的stETH供应量差。
        steth_supply_change_pct = steth_supply_change / yesterday_real_8am["steth_supply_eth"] * 100

        #日环比数据组装成TG消息发送
        tg_daily_msg = (#把要发到tg的日环比相关数据按以下格式布置
            f"📅 *Lido 日环比数据简报*\n"
            f"{'─'*28}\n"
            f"昨  {yesterday_real_8am['timestamp']}\n"
            f"今  {today_real_8am['timestamp']}\n"
            f"{'─'*28}\n"
            f"*[ ETH 价格 ]*\n"
            f"Chainlink  ${yesterday_real_8am['eth_price_chainlink']:,.2f}  →  *${today_real_8am['eth_price_chainlink']:,.2f}*"
            f"   {arrow(eth_change_chainlink_real_8am)} *{eth_change_chainlink_real_8am:+,.2f}* ({eth_change_chainlink_percentage_real_8am:+.2f}%)\n"
            f"Pyth       ${yesterday_real_8am['eth_price_pyth']:,.2f}  →  *${today_real_8am['eth_price_pyth']:,.2f}*"
            f"   {arrow(eth_change_pyth_real_8am)} *{eth_change_pyth_real_8am:+,.2f}* ({eth_change_pyth_percentage_real_8am:+.2f}%)\n"
            f"\n"
            f"*[ stETH 供应量 ]*\n"
            f"昨 {yesterday_real_8am['steth_supply_eth']:,.2f} ETH  →  今 *{today_real_8am['steth_supply_eth']:,.2f} ETH*\n"
            f"{arrow(steth_supply_change)} *{steth_supply_change:+,.2f} ETH*  ({steth_supply_change_pct:+.4f}%)\n"
            f"\n"
            f"*[ TVL 日环比 ]*\n"
            f"Chainlink  ${yesterday_real_8am['total_tvl_chainlink']:,.2f}  →  *${today_real_8am['total_tvl_chainlink']:,.2f}*\n"
            f"  {arrow(tvl_change_chainlink_real_8am)} *${tvl_change_chainlink_real_8am:+,.2f}*  ({tvl_change_chainlink_percentage_real_8am:+.2f}%)\n"
            f"DefiLlama  ${yesterday_real_8am['Defillama_tvl']:,.2f}  →  *${today_real_8am['Defillama_tvl']:,.2f}*\n"
            f"  {arrow(defillama_change_tvl_real_8am)} *${defillama_change_tvl_real_8am:+,.2f}*  ({defillama_change_tvl_percentage_real_8am:+.2f}%)\n"
            f"预警：*{daily_tvl_alert}*\n"
            f"\n"
            f"*[ APR 日环比 ]*\n"
            f"{daily_apr_line}\n"
            f"{'─'*28}\n"
            f"*[ 日变化汇总 ]*\n"
            f"ETH价格   {arrow(eth_change_chainlink_real_8am)} *${eth_change_chainlink_real_8am:+,.2f}*   ({eth_change_chainlink_percentage_real_8am:+.2f}%)\n"
            f"供应量    {arrow(steth_supply_change)} *{steth_supply_change:+,.2f} ETH*  ({steth_supply_change_pct:+.4f}%)\n"
            f"总TVL     {arrow(tvl_change_chainlink_real_8am)} *${tvl_change_chainlink_real_8am:+,.2f}*  ({tvl_change_chainlink_percentage_real_8am:+.2f}%)\n"
            f"{'─'*28}"
        )
        print(tg_daily_msg)#打印tg_daily_msg，方便我们查看。

        #以下是AI分析日环比过程的代码
        if hour == 8:#如果现在的时间是早上八点整，那么就执行下面的代码。
            send_tg(tg_daily_msg)#只在早上八点发送日环比简报到tg。
...
"""
            ...
            gpt_daily_result = call_gpt(messages=[
                    {"role": "system",
                    "content": "你是一个专业的DeFi数据分析师，擅长分析Lido协议的TVL和ETH价格走势。请严格按照用户要求的格式输出，不要添加多余内容。"},
                    {"role": "user",
                    "content": f"""{report_text}
请分析以上Lido协议日环比数据，严格按照以下格式输出，不要偏离格式：
...
• ETH价格：说明涨跌幅及方向
• TVL变化：说明涨跌幅及方向
• 联动关系：价格与TVL是否同向，说明原因

【资金信号】
说明资金是流入还是流出，用户是在质押还是赎回，一到两句话。

【风险提示】
风险等级：🟢低 / 🟡中 / 🔴高
说明理由，一到两句话。
"""}#规定好ai分析以后内容输出的格式，让gpt按照这个格式输出。
                ],
                model="gpt-5.5"
            )
            if gpt_daily_result is not None:#如果gpt_daily_result不为空，就执行下面步骤。
                print(gpt_daily_result)#打印ai分析的回答数据。
                tg_gpt_daily_msg = (#把要发到tg的ai分析相关数据按以下格式布置
                    f"🤖 *Lido 日报 · AI 分析*\n"
                    f"{'─'*28}\n"
                    f"{today_real_8am['timestamp']}\n"
                    f"{'─'*28}\n"
                    f"{gpt_daily_result}\n"
                    f"{'─'*28}"
                )
                send_tg(tg_gpt_daily_msg)#把这些数据产生放进send_tg函数里面，然后发到tg。   

            #以下是把通过gpt分析的日环比变化数据存到对应json文件的代码。
            daily_result_filename = os.getenv("DAILY_FILE")#使用os.getenv函数从.env文件里拿到日报记录文件的文件名。

            if os.path.exists(daily_result_filename):#如果文件存在，就执行下面步骤。
                with open(daily_result_filename, "r", encoding="utf-8") as file:#用只读方式把硬盘里这个文件里存储的数据放到内存里等待加工。
                    daily_history = json.load(file)#在内存里用json翻译官把数据翻译成python可读的形式待用。  
            else:#如果文件不存在，就创建一个空列表待用。
                daily_history = []#如果文件不存在，就创建一个空列表待用。
            daily_history.append({#在内存里把本次运行的新数据按以下格式加到到翻译过的原数据的后面。还是像上面一样分两种情况。
                "timestamp": today_real_8am["timestamp"],#把本次运行的时间戳数据加到列表里面。
                "report": report_text,#把本次运行的日环比数据加到列表里面。
                "gpt_analysis": gpt_daily_result#把本次运行的ai分析数据加到列表里面。
            })

            with open(daily_result_filename, "w", encoding="utf-8") as file:#用可写方式打开硬盘里面储存的文件。
                json.dump(daily_history, file, ensure_ascii=False, indent=2)#把内存里加工过的文件用json翻译官翻译回json格式写回到硬盘中去。
    ...
    #周报分析gpt的调用过程
        gpt_week_result = call_gpt(messages=[#调用gpt的api接口，把messages列表里面的内容按顺序一条一条的传给gpt，gpt根据这些内容生成回答。  
                {"role": "system",
                "content": "你是一个专业的DeFi数据分析师，擅长分析Lido协议的TVL和ETH价格走势。请严格按照用户要求的格式输出，不要添加多余内容。"},
                {"role": "user",
                "content": f"""{week_report_text}
请分析以上Lido协议过去七天的数据，严格按照以下格式输出，不要偏离格式：

【本周总结】
用一句话概括本周整体情况。

【ETH价格走势】
说明本周价格整体方向、最大涨跌幅及关键转折点，两到三句话。

【TVL走势】
说明本周TVL整体方向及累计变化幅度，两到三句话。
...
【本周市场情绪】
🟢乐观 / 🟡中性 / 🔴悲观，说明判断依据，一到两句话。

【下周风险点】
列出1到2个需要重点关注的风险，每条一句话。
"""}
            ],
            model="gpt-5.5"
        )#这几个括号就表示又内到外把函数公式一一对应运行关闭，和excel里的嵌套函数是一个原理。
        if gpt_week_result is not None:#如果gpt_week_result不是空集，就是说只要不是api没连上，或者连上了没返回回答的情况下，就执行以下步骤。
            ...
            send_tg(tg_gpt_week_msg)#发送tg消息。

    #下面是把gpt分析的周报存入对应json文本的过程
        week_result_filename = os.getenv("WEEK_FILE")#用week_result_filename定义周报文件路径。  
...
#定时调度：每小时整点执行一次job函数。
schedule.every().hour.at(":00").do(job)#规定每个小时00分的时候执行一次job函数。

job()#程序启动时立刻先运行一次，进行效果调试时使用。

while True:#让程序一直保持运行状态，每60秒检查一次有没有到达执行时间。
    schedule.run_pending()#检查有没有到达执行时间。
    time.sleep(60)#让程序每60秒检查一次有没有到达执行时间。
