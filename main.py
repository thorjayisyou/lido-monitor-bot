...
def job():#把所有业务逻辑包进job函数，schedule每小时调用一次这个函数。
    ...
    #先计算小时TVL变化，再一起打进TG消息
    if len(history1)<2:#历史数据不足，跳过小时预警分析
        print("历史数据不足，跳过小时预警分析")
        hourly_change_line = "历史数据不足，暂无小时变化数据"
    else:
        ...
        if tvl_hourly_change_percentage<-0.5:
            ...
            alert = f"🟡可能存在风险，需要注意了。"
        else:
            alert = f"✅数据正常，该吃吃该喝喝。"
        print(f"链上总锁仓量在过去1小时内变化：{alert}")
        hourly_change_line = (
            f"变化额：*${tvl_hourly_change:+,.2f}  ({tvl_hourly_change_percentage:+.2f}%)*\n"
            f"预警：*{alert}*"
        )
    ...
    tg_hourly_msg = (
        ...
        f"{'─'*28}\n"
        f"*[ Chainlink ]*\n"
        f"ETH 价格 : *${latest['eth_price_chainlink']:,.2f}*\n"
        f"总 TVL   : *${latest['total_tvl_chainlink']:,.2f}*\n"
        f"\n"
        f"*[ Pyth ]*\n"
        f"ETH 价格 : *${latest['eth_price_pyth']:,.2f}*\n"
        f"总 TVL   : *${latest['total_tvl_pyth']:,.2f}*\n"
        f"\n"
        f"*[ DefiLlama ]*\n"
        f"总 TVL   : *${latest['Defillama_tvl']:,.2f}*\n"
        f"{'─'*28}\n"
        f"*[ 小时 TVL 变化 ]*\n"
        f"{hourly_change_line}\n"
        f"{'─'*28}"
    )
    print(tg_hourly_msg)
    ...
    if today_real_8am is None or yesterday_real_8am is None:#如果几天或者昨天的时间锚定点里面任中返回一个空集，就是说有数据找不到，那么就没办法进行日环比计算。
        ...
        tg_daily_msg = (
            f"📅 *Lido 日环比数据简报*\n"
            f"{'─'*28}\n"
            f"昨天  {yesterday_real_8am['timestamp']}\n"
            f"今天  {today_real_8am['timestamp']}\n"
            f"{'─'*28}\n"
            f"*[ ETH 价格日环比（Chainlink） ]*\n"
            f"昨天 : ${yesterday_real_8am['eth_price_chainlink']:,.2f}\n"
            f"今天 : *${today_real_8am['eth_price_chainlink']:,.2f}*\n"
            f"变化 : *${eth_change_chainlink_real_8am:+.2f}  ({eth_change_chainlink_percentage_real_8am:+.2f}%)*\n"
            f"\n"
            f"*[ TVL 日环比（Chainlink） ]*\n"
            f"昨天 : ${yesterday_real_8am['total_tvl_chainlink']:,.2f}\n"
            f"今天 : *${today_real_8am['total_tvl_chainlink']:,.2f}*\n"
            f"变化 : *${tvl_change_chainlink_real_8am:+,.2f}  ({tvl_change_chainlink_percentage_real_8am:+.2f}%)*\n"
